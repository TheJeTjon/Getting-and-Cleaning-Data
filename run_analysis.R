...
# clearer names using  find and replace
names(x_merged) <- gsub('Acc',"Acceleration",names(x_merged))
names(x_merged) <- gsub('GyroJerk',"AngularAcceleration",names(x_merged))
names(x_merged) <- gsub('Gyro',"AngularSpeed",names(x_merged))
...
names(x_merged) <- gsub('Freq\\.',"Frequency.",names(x_merged))
names(x_merged) <- gsub('Freq$',"Frequency",names(x_merged))
